...
namespace iWms.Form
{
    ...
    /// </summary>
    public partial class RowMergeDataGridView : DataGridView
    {
        #region 构造函数
        public RowMergeDataGridView()
        {
            InitializeComponent();
        }
        #endregion
        ...
        private void DrawCell(DataGridViewCellPaintingEventArgs e)
        {
            ...
            if (this.MergeColumnNames.Contains(this.Columns[e.ColumnIndex].Name) && e.RowIndex != -1)
            {
                ...
                if (!string.IsNullOrEmpty(curValue))
                {
                    #region 获取下面的行数
                    //当前行的合并单元格参考列数据
                    string curFocusValueCombine = string.Empty;
                    foreach (var focusName in MergeFocusNames)
                    {
                        curFocusValueCombine = $"{curFocusValueCombine}/{Rows[e.RowIndex].Cells[focusName].Value}";
                    }
                    for (int i = e.RowIndex; i < this.Rows.Count; i++)
                    {
                        string thisRowFocusValue = string.Empty;
                        foreach (var focusName in MergeFocusNames)
                        {
                            thisRowFocusValue = $"{thisRowFocusValue}/{Rows[i].Cells[focusName].Value}";
                        }
                        if (this.Rows[i].Cells[e.ColumnIndex].Value.ToString().Equals(curValue)
                         && curFocusValueCombine.Equals(thisRowFocusValue))
                        {
        ...
        public List<string> MergeFocusNames { get; set; } = new List<string>();
        #endregion
        #region 二维表头
