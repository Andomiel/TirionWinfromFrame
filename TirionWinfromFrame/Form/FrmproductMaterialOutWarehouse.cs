...
namespace MES.Form
{
	public partial class FrmproductMaterialOutWarehouse : FrmBaseForm
	{
        ...
        {
			InitFrom(xtraTabControl1,grdList,grdListView,new LayoutControlGroup[]{layoutControlGroup1},new productMaterialOutWarehouseInfo(),gridControl1,new []{"txtcode"});
            InitSearchDicData();
                ...
                using (var db = new MESDB())
                {
                    ...
                    if (!string.IsNullOrEmpty(code))
                    {
                        ...
                        else
                        {
                            ...
                            gridView1.GetFocusedDataRow()["materialtype"] = buyerdetailInfo.materialtype;
        ...
        {
            string code = "MMO" + DateTime.Now.GetDateTimeCode();
            ...
            try
            {
                ...
                using (var db = new MESDB())
                {
                    using (var tran = db.Database.BeginTransaction())
                    {
                        try
                        {
                            Dictionary<string, List<productMaterialOutWarehouseDetailInfo>> dic =
                                dt.GetDataTableData<productMaterialOutWarehouseDetailInfo>();
                            if (info.id == 0) //新增
                            {
                                info.code = code;
                                db.productMaterialOutWarehouseInfo.Add(info);
                                db.SaveChanges();
                                txtid.Text = info.id.ToString();
                                txtcode.Text = code;
                                if (dt != null)
                                {
                                    List<productMaterialOutWarehouseDetailInfo> detaiListAdd =
                                        dic["Add"];
                                    int num = 0;
                                    ...
                                    db.productMaterialOutWarehouseDetailInfo.AddRange(detaiListAdd);
                                    db.SaveChanges();
                                }
                            }
                            else //更新
                            {
                                ...
                                if (dt != null)
                                {
                                    ...
                                    List<productMaterialOutWarehouseDetailInfo> detaiListEdit =
                                        dic["Edit"];
                                    ...
                                    List<productMaterialOutWarehouseDetailInfo> detaiListDel =
                                        dic["Del"];
                            ...
                            tran.Commit();
                            gridControl1.DataSource = db.productMaterialOutWarehouseDetailInfo.ToList().Where(p => p.masterid == info.id); ; ;
                        }
                        catch (Exception ex)
                        {
                            tran.Rollback();
		...
		public override bool DelFunction()
        {
            try
            {
                ...
                using (var db = new MESDB())
                {
                    ...
                    List<productMaterialOutWarehouseDetailInfo> list = db.productMaterialOutWarehouseDetailInfo
                        .Where(p => p.masterid == info.id).ToList();
                    db.productMaterialOutWarehouseDetailInfo.RemoveRange(list);
        ...
        {
            productMaterialOutWarehouseInfo info = grdListView.GetFocusedRow() as productMaterialOutWarehouseInfo;
            if (info != null)
            {
                using (var db = new MESDB())
                {
                    gridControl1.DataSource = db.productMaterialOutWarehouseDetailInfo.Where(p => p.masterid == info.id).ToList().ToDataTable();
        ...
        {
            gridControl1.DataSource = new List<productMaterialOutWarehouseDetailInfo>().ToDataTable();
        }
